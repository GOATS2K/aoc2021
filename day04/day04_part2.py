class Bingo:
    ...
    def get_puzzle_input(self) -> list[str]:
        with open("day04/test_input.txt", "r") as infile:
            lines = []
    ...
    def construct_boards(self) -> dict[int, list[list[str]]]:
        with open("day04/test_boards.txt", "r") as infile:
        ...
        for bingo_number in self.input:
            ...
            for board_number, row_locations in results.items():
                ...
                if self.scan_column(self.boards[board_number], board_number):
                    if bingo_number not in seen_winning_numbers:
                        winners.append(
                            {
                        ...
                        seen_winning_numbers.append(bingo_number)
...
if __name__ == "__main__":
    ...
    last_winning_board = winning_board[-1]
    print(last_winning_board)
    sum_of_tiles = 0
    for row in last_winning_board["board"]:
        for tile in row:
            if tile != -1:
                sum_of_tiles += int(tile)
    print(f"Answer: {sum_of_tiles * int(last_winning_board['winning_number'])}")
